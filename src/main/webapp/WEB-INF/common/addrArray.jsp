...
//지역 설정
<%
	String[] addr0 = {"시/도 선택","서울특별시","인천광역시","대전광역시","광주광역시","대구광역시","울산광역시","부산광역시","경기도","강원도","충청북도","충청남도","전라북도","전라남도","경상북도","경상남도","제주도"};
%>
	...
	var s_selarr = [
	 ...
	 function changeSelect(f) {
		$("#hideon").show();
		var s_sel = f.address1; // 리셋
		for (var i = s_sel.length-1; i > 0; i--) {
			s_sel.options[i] = null;
		}
		
		var findex = f.address1.selectedIndex;
...
<select name="address1" id="address1" onchange="changeSelect(myform)">
	<c:forEach var="i" items="<%=addr0 %>">
	<option value="${i }">${i }
	</c:forEach>
</select>
</div>
